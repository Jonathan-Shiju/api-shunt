...
// --- Authentication API Provider 1 ---
app.post("/auth-1/login", (req, res) => {
  // Parameters: username, password
  const { username, password } = req.body;
  // ...dummy logic...
  res.json({
    message: "Dummy login successful (Provider 1)",
    received: { username, password },
  });
});

// --- Authentication API Provider 2 ---
app.post("/auth-2/login", (req, res) => {
  // Parameters: email, passcode
  const { email, passcode } = req.body;
  // ...dummy logic...
  res.json({
    message: "Dummy login successful (Provider 2)",
    received: { email, passcode },
  });
});
...
app.listen(PORT, () => {
  console.log(`Dummy Auth API server running on port ${PORT}`);
});
